...
- Karten: `views/mapeditor.js` (Dungeon‑Editor, Typ `scrawl`: Formen → Maske → Wände/Schraffur per Dilatation, Objekte als Vektorzeichnungen, Generatoren, Export, Spielmodus; `buildGrid()` = begehbare Felder, schwieriges Gelände, `opaque` (Wände blockieren Sicht), `cover` (Säulen); `s.clampView()` hält die Karte beim Verschieben/Zoomen im Bild – ebenso in `maps.js`) · `views/maps.js` (Liste, Welt‑/Rasterkarten, Weiche `MapView`).
