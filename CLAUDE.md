...
- Karten (Typ `scrawl`): `views/mapeditor.js` (Werkzeuge in der **linken** Seitenleiste, Ebenenliste rechts, Generatoren, Spielmodus) + `views/maprender.js` (Darstellung) + `core/userassets.js` (eigene Pakete).
  ...
  - Objekte: `{ t: 'stamp', a: '<quelle>:<id>', x, y, r, s, fx?, layer?, sh?, o? (Deckkraft), b? (Weichzeichnen), hidden? }` – Quellen `ph:` (Poly Haven, `data/mapassets.js`), `p:` (prozedurale Bauteile aus `PROC`), `u:` (eigene, nur lokal). Alte Symbole (`OBJ`) werden über `LEGACY` auf Stempel abgebildet. Ebenen: `floor` < `obj` < `top`; Schatten und Leuchten kommen aus `assetInfo()`.
  ...
  - Lücken per KI füllen (lokal, ohne Kosten): `tools/sdxl/` – eigenständige Python-Umgebung (uv + PyTorch cu128), `gen.py` erzeugt Texturen **nahtlos** (circular padding) und Objekte auf Magenta mit automatischem Freistellen; Auftragsliste `jobs.json`. Ergebnisse landen in `tools/sdxl/out/` und werden erst nach Sichtung übernommen. Alles außer `gen.py`/`jobs.json` ist in `.gitignore`.
...
- Dateiexplorer (`views/codex.js`): Sortierung **Eigene Reihenfolge** (`settings.explorerOrder[cid][ordnerpfad] = [schlüssel]`, Schlüssel: Notiz‑Id, `f:<pfad>`, `x:<dateiId>`), Ziehen & Ablegen verschiebt (in Ordner hinein) oder sortiert ein (obere/untere Hälfte der Zeile); am Rand scrollt die Liste mit.
...
- **Freigabe an einzelne Spieler**: `visFields(visibility, only)` schreibt `{ visibility, only: [uid…], onlyN }`; leeres `only` = alle Spieler. Spieler lesen mit zwei Abfragen (`visQueries(uid)`: `onlyN == 0` **und** `only array-contains uid`) – die Regel `sharedWithMe()` prüft dasselbe. Ältere Dokumente rüstet `ensureVisibilityFields(cid)` beim Öffnen als SL nach. UI: `shareDialog()`/`shareNoteDialog()` in `views/codex.js`, Karten über die Karteneinstellungen.
